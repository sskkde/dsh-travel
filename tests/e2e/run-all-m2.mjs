/**
 * M2 W9 收口验收 runner（M1 run-all 的 m2 版）：① 12306 MCP 存活核验 →
 * ② 剧本 A~F 顺序执行（A~E 沿用 M1 剧本；F=M2 有 key 增强版：机票三档+
 *    cityTransfer+playwright 渠道+B 模式渲染）→ ③ 聚合结果 + 矩阵核对表。
 * 证据落 docs/evidence/m2/final/（TRAVEL_EVID_MILESTONE=m2 由本入口设置）。
 *
 * 用法：node tests/e2e/run-all-m2.mjs [--skip-build]
 */
...
function bundleAndRun(name) {
  ...
  const t0 = Date.now()
  const run = spawnSync('node', [bundleOut], { cwd: repoRoot, encoding: 'utf8', timeout: Number(process.env['E2E_SCRIPT_TIMEOUT_MS'] ?? 900_000) })
  const durationMs = Date.now() - t0
  const out = join(EVID, name, 'run-output.log')
  mkdirSync(join(EVID, name), { recursive: true })
...
async function main() {
  ...
  process.env['TRAVEL_EVID_MILESTONE'] ??= 'm2' // 外部已设（如 M3 runner banner=m3）则尊重，避免覆写 M2 归档
  transcript.log('M2 W9 收口验收 runner 开始（剧本 A~F；F=有 key 增强版）')
